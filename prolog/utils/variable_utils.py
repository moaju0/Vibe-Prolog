"""Utility functions for manipulating variables inside Prolog terms."""
...
def collect_vars_in_order(term: Any, subst: Substitution, seen: set[str] | None = None) -> list[str]:
...
def copy_term_recursive(term: Any, var_map: dict, fresh_variable: Callable[[str], Variable]) -> Any:
    ...
    if isinstance(term, Compound):
        new_args = tuple(copy_term_recursive(arg, var_map, fresh_variable) for arg in term.args)
        return Compound(term.functor, new_args)
    if isinstance(term, List):
        new_elements = tuple(copy_term_recursive(elem, var_map, fresh_variable) for elem in term.elements)
        new_tail = copy_term_recursive(term.tail, var_map, fresh_variable) if term.tail is not None else None
